...
addpath('/home/david/Documents/RRET/samruddhi_scripts/HBML_JH_iEEG_preproc')
...
%%
   ...
   %%
    ...
    if exist ("Pressure",'var')
        ...
        % ts_start = timestampsSL;
        % ts_stop  = timestampsSL_stop;
        % t = (0:length(Pressure)-1)/100;
        % figure
        % plot(t, Pressure)
        % hold on
        % for i = 1:length(timestampsSL)
        %     xline(ts_start(i), 'g');
        %     xline(ts_stop(i), 'r');
        % end
        % legend({'Pressure','Start','Stop'})
        % xlabel('Time (s)')
        % ylabel('Amplitude')
        % title('Pressure with SL timestamps')
...
if addETnow
    ...
    params = ecog.params;
    [exp_log_path, ~, ~] = fileparts(params.smartlab.sinktimes);
    trialsinfo = readtable(exp_log_path, 'Sheet', [params.filename(1:end) '_trialsinfo']);
