...
fit_model_with_fallback <- function(df, tol_sing = 1e-4, maxfun = 2e5) {
  ...
  forms <- list(
    lmer_sujet_chan = Pxx ~ A_scaled * R_scaled * C_scaled + (1 | chan),                
    lmer_sujet  = Pxx ~ A_scaled * R_scaled * C_scaled + (1 | chan),                     
    lm_fixed    = Pxx ~ A_scaled * R_scaled * C_scaled                                   
  )
...
outputdir_fig = "/home/jules/Documents/VCCP_JULES/Analyses/results/Pxx/LMM/diagnosis"
outputdir_df_lmm = "/home/jules/Documents/VCCP_JULES/Analyses/precompute/EXPORT_DF/patient_wise/Pxx/LMM_res/REG"
...
for (band in band_list) {
  ...
  filename <- paste0("/", sujet, "_df_reg_Pxx_", band, "_VCCP_R.xlsx")
  ...
  for (phase_cycle_sel in phase_list) {
    ...
    for (ROI_sel in ROI_list) {
      ...
      tryCatch({
        ...
        df <- df_oneROI[, c("sujet", "chan", "cycle", "Pxx", "A_scaled", "R_scaled", "C_scaled")]
...
fit_model_with_fallback <- function(df, tol_sing = 1e-4, maxfun = 2e5) {
  ...
  forms <- list(
    lmer_sujet_chan = Pxx ~ A_scaled * R_scaled * C + (1 | chan),                
    lmer_sujet  = Pxx ~ A_scaled * R_scaled * C + (1 | chan),                     
    lm_fixed    = Pxx ~ A_scaled * R_scaled * C                                   
  )
  ...
  for (phase_cycle_sel in phase_list) {
    ...
    for (ROI_sel in ROI_list) {
      ...
      tryCatch({
        ...
        # Keep only required columns
        df <- df_oneROI[, c("sujet", "chan", "cycle", "Pxx", "A_scaled", "R_scaled", "C")]
...
fit_model_with_fallback <- function(df, tol_sing = 1e-4, maxfun = 2e5) {
  ...
  forms <- list(
    lmer_sujet_chan = Pxx ~ A_scaled * R_scaled * cond + (1 | chan),                
    lmer_sujet  = Pxx ~ A_scaled * R_scaled * cond + (1 | chan),                     
    lm_fixed    = Pxx ~ A_scaled * R_scaled * cond                                   
  )
...
for (band in band_list) {
  ...
  filename <- paste0("/", sujet, "_df_reg_Pxx_", band, "_attention_R.xlsx")
  df_raw <- read_excel(paste(root, filename, sep = "/"))
  ...
  for (phase_cycle_sel in phase_list) {
    ...
    for (ROI_sel in ROI_list) {
      ...
      tryCatch({
        ...
        # Keep only required columns
        df <- df_oneROI[, c("sujet", "chan", "cycle", "Pxx", "A_scaled", "R_scaled", "cond")]
        ...
        df$cond  <- relevel(df$cond,  ref = "RB")
