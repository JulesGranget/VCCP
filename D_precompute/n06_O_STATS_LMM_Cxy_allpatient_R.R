...
fit_model_with_fallback <- function(df, tol_sing = 1e-4, maxfun = 2e5) {
  ...
  forms <- list(
    lmer_sujet_chan = Cxy ~ A * R * C + (1 | chan),                
    lmer_sujet  = Cxy ~ A * R * C + (1 | chan),                     
    lm_fixed    = Cxy ~ A * R * C                                    
  )
...
root = "/home/jules/Documents/VCCP_JULES/Analyses/precompute/EXPORT_DF/patient_wise/Cxy"
outputdir_fig = "/home/jules/Documents/VCCP_JULES/Analyses/results/Cxy/LMM/VCCP/patientwise/diagnosis"
outputdir_df_lmm = "/home/jules/Documents/VCCP_JULES/Analyses/precompute/EXPORT_DF/patient_wise/Cxy/LMM_res"
sujet = "LH018"
protocol_type = "VCCP"


filename <- paste0("/", sujet, "_df_Cxy_protocol_R.xlsx")
...
for (ROI_sel in ROI_list) {
  ...
  tryCatch({
    ...
    filename_export_diagnostic <- paste(protocol_type, ROI_sel, sep = "_")
    ...
    df <- df_oneROI[, c("sujet", "chan", "Cxy", "A", "R", "C")]
    ...
    # ------------------------------------------------------------
    p_box <- ggplot(df, aes(x = sujet, y = Cxy, color = sujet, fill = sujet)) +
      geom_boxplot(width = .2, alpha = .5, outlier.alpha = 0,
                    position = position_dodge(.9)) +
      stat_summary(fun = median, geom = "point", size = 2,
                    position = position_dodge(.9), color = "white") +
      labs(
        title = paste(filename_export_diagnostic, "Cxy — Subject-wise", sep = " | ")
      ) +
      theme(
    ...
    # SAVE one single PNG
    # ------------------------------------------------------------
    file_diag <- paste("DIAGNOSTIC", filename_export_diagnostic, "Cxy.png", sep = "_")
    ...
    filesxlsx_ROI <- paste("RES", filename_export_diagnostic, "Cxy.xlsx", sep = "_")
